...
import { mapboxglAccessToken, InitialConfigMapboxglUS } from "../../const/constants"
    ...
    'features': [
      {
        ...
        'properties': {
          'title': solutionToDisplay[0][2] + ', ' + solutionToDisplay[0][3] + ' - Origin city'
        }
      }
    ]
  }
  ...
  mapboxglMap.on('load', () => {
    ...
    mapboxglMap.loadImage(
      ...
      (error, image) => {
        ...
        mapboxglMap.addSource('origin', returnOriginCitySourceConfig(solutionToDisplay));
...
const updateRouteLayer = (displaySolution) => {
  mapboxglMap.getSource("route").setData(returnRouteSourceDataConfig(displaySolution))
...
const updateOriginCityLayer = (displaySolution) => {
  ...
  mapboxglMap.getSource("origin").setData(returnOriginCityLayerDataConfig(displaySolution));
  mapboxglMap.addLayer(returnOriginCityLayerConfig());
}
