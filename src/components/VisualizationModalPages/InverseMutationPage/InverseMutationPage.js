...
import {
  generateShuffledArrayOfAllLocations,
...
const InverseMutationPage = () => {
  ...
  const [partBeforeInversedPart, setPartBeforeInversedPart] = useState(
  ...
  const [inversionBoundariesGenerated, setInversionBoundariesGenerated] =
    useState(false);
  ...
  const choseAndMarkPartToBeInversed = (solution) => {
    ...
    setPartToBeInversed(
      solution.slice(start, end + 1).map((location) => ({
        ...location,
            ...
            <div>
              {inversionBoundariesGenerated ? (
                inversionDone ? null : (
                  <Button
                    variant="primary"
                    onClick={() => inverse(partToBeInversed)}
                ...
                <Button
                  variant="primary"
                  onClick={() =>
                    choseAndMarkPartToBeInversed(partBeforeInversedPart)
                  }
                  style={{ margin: "1rem", marginTop: "1.5rem" }}
                >
