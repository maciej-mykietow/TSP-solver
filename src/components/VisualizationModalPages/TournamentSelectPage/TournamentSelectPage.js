...
import {
  generateArrayOfRandomSolutionsWithFitnessInRange,
...
const selectAndSortSolutionsInTournament = (solutionsInCurrentGeneration) => {
  return solutionsInCurrentGeneration
    .filter((solution) => {
...
const markSolutionsInTournamentAsMarked = (
  solutionsInCurrentGeneration,
  indexesOfSolutionsInTournament
) => {
...
const TournamentSelectPage = () => {
  ...
  const [solutionsInCurrentGeneration] = useState(
    generateArrayOfRandomSolutionsWithFitnessInRange(
      populationSize,
      minFitness,
      maxFitness
  ...
  const selectSolution = (
    solutionsInCurrentGeneration,
    solutionsInNewGeneration
  ) => {
    if (solutionsInNewGeneration.length == solutionsInCurrentGeneration.length)
      return;

    let indexesOfSolutionsInTournament = generateArrayOfRandomNumbersInRange(
      tournamentSize,
      populationSize
    );
    markSolutionsInTournamentAsMarked(
      solutionsInCurrentGeneration,
      indexesOfSolutionsInTournament
    );
    let solutionsInTournament = selectAndSortSolutionsInTournament(
      solutionsInCurrentGeneration
    );
    ...
    let updatedNewGeneration = solutionsInNewGeneration.concat([
      {
        id: solutionsInNewGeneration.length,
        value: solutionsInTournament[0].value,
      },
    ]);
    ...
    return solutions.map((solution, i) => {
      return (
        <FlipMoveListItem
          key={solution.id}
          listItemClass={
            solution.inTournament
              ? solution.bestInTournament
                ? "solutionItem InTournament"
                : "solutionItem InTournamentSelected"
              : "solutionItem"
          }
          index={i}
          {...solution}
      ...
      <Row>
        <Col className="page-column" lg="12">
          ...
          <Row
            ...
            style={{ height: "20%" }}
          >
            {solutionsInCurrentGeneration.length ==
            solutionsInNewGeneration.length ? null : (
              <Button
                className="select-solution-button"
                variant="primary"
                onClick={() =>
                  selectSolution(
                    solutionsInCurrentGeneration,
