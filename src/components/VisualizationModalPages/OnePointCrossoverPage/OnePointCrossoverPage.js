...
const generateFirstParent = (tspProblemSize, color) => {
  return markFirstHalfOfSolution(
    generateShuffledArrayOfAllLocations(tspProblemSize),
    color
  );
};

const generateSecondParent = (tspProblemSize, color) => {
  return markSecondHalfOfSolution(
    generateShuffledArrayOfAllLocations(tspProblemSize),
    color
  );
};
...
const markFirstHalfOfSolution = (solution, color) => {
  return solution.map((obj, i) => ({
    ...obj,
    backgroundColor: i < solution.length / 2 ? color : "",
  }));
};
...
const OnePointCrossoverPage = () => {
  ...
  const [firstParentSolution] = useState(
    generateFirstParent(tspProblemSize, "gold")
  );
  const [secondParentSolution] = useState(
    generateSecondParent(tspProblemSize, "sandybrown")
  );
  ...
  const performCrossover = (firstParentSolution, secondParentSolution) => {
    setCreatedSolution([
      ...firstParentSolution
        .slice(0, tspProblemSize / 2)
        .map((location, i) => ({
          id: i,
          value: location.value,
          backgroundColor: location.backgroundColor,
        })),
      ...secondParentSolution
        .slice(tspProblemSize / 2, tspProblemSize)
        .map((location, i) => ({
  ...
  const isLocationDuplicated = (index, locationsInSolution, location) => {
    ...
    return index < tspProblemSize / 2
      ? false
      : locationsInSolution.indexOf(location.value) !=
          locationsInSolution.lastIndexOf(location.value);
  };
  ...
  const checkSolution = () => {
    ...
    setCreatedSolution([
      ...createdSolution.map((location, i) => ({
        ...location,
        backgroundColor: isLocationDuplicated(i, locationsInSolution, location)
          ? "red"
          : "",
      })),
    ]);
  ...
  const correctSolution = (createdSolution) => {
    ...
    let missingLocations = allLocationsSequence.filter(
      (location) => !solutionWithNoDuplicates.includes(location)
    );
    let correctedSolution = [
      ...solutionWithNoDuplicates.map((value, i) => ({
        id: i,
        value: value,
      })),
    ...
    <Modal.Body>
      <Row>
        <Col className="page-column" lg="12">
          <Row
            className="fade-in-image description-field"
            style={{ height: "45%" }}
          >
          ...
          <Row
            className="fade-in-image interactive-field"
            style={{ height: "12.5%" }}
          >
            <FlipMoveWrapper
              style={{ paddingInlineStart: "1px", marginBottom: "0px" }}
            >
          ...
          >
            {solutionCreated ? null : (
              <Button
                ...
                variant="primary"
                onClick={() =>
                  performCrossover(firstParentSolution, secondParentSolution)
                }
              >
                Perform crossover
              </Button>
          ...
          >
            ...
            {solutionChecked && !solutionCorrected ? (
              <Button
                className="crossover-page-button"
                variant="primary"
                onClick={() => correctSolution(createdSolution)}
              >
                Correct solution
              </Button>
            ) : null}
