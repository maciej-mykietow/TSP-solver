...
const RandomSolutionPage = () => {
  ...
  const [tspSolution, setTspSolution] = useState(
    generateArrayOfAllLocations(tspProblemSize)
  );
  ...
  const renderSolution = () => {
    return tspSolution.map((location) => {
      return <FlipMoveListItem key={location.id} index={location.id} />;
    });
  };
        ...
        <Col className="page-column" lg="12">
          ...
          >
            <div>
              <Button
                variant="primary"
                onClick={shuffleSolution}
                style={{ margin: "1rem", marginTop: "1.5rem" }}
              >
                {"Shuffle"}
              </Button>

              <FlipMoveWrapper>{renderSolution()}</FlipMoveWrapper>
            </div>
          </Row>
        </Col>
      </Row>
